...
public class JdbcMealRepository implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            Number mealId = simpleJdbcInsert.executeAndReturnKey(parameterSource);
            meal.setId(mealId.intValue());
        } else if (namedParameterJdbcTemplate.update("UPDATE meals SET " +
                        "datetime=:datetime, description=:description, calories=:calories " +
    ...
    public List<Meal> getAll(int userId) {
        return getBetweenHalfOpen(LocalDateTime.MIN, LocalDateTime.MAX, userId);
    }
